...
exports.recommendationByLocation = async (req, res) => {
  const requirement =
    "Select the top three restaurants from the json array below and reply me using this JSON schema:\n Restaurant = {'name': string, 'vicinity': string, 'photo_reference':string, 'rating':string}\n Return: Array<Restaurant>`;\n ";
  ...
  try {
    ...
    const prompt = requirement + JSON.stringify(restaurants);
    console.log(prompt);
    console.log("-------------------------------------------------");
    const result = await model.generateContent(prompt);
    console.log(result.response.text());
    return res.json({ restaurants: result.response.text() });
  } catch (error) {
